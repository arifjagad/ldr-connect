...
function DareDerbyContent() {
  ...
  if (phase === "idle") {
    return (
      <div className="mx-auto w-full max-w-md px-4 py-10 flex flex-col gap-6">
        <div>
          <Link href="/dashboard/games" className="text-xs text-[#5C5470] hover:text-[#9B93B0] transition">
            ← Kembali
          </Link>
          <h1 className="mt-3 text-3xl font-bold text-[#FFF5F8]">Dare Derby 🎲</h1>
          <p className="mt-1 text-sm text-[#9B93B0]">Mini-game kompetitif — yang kalah dapat dare dari pasangan!</p>
        </div>
        ...
        <div className="rounded-2xl border border-[#FF3D7F]/25 bg-[#FF3D7F]/5 p-5 flex flex-col gap-4">
          <h2 className="font-semibold text-[#FFF5F8]">Buat Sesi Baru</h2>
          <button
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5 flex flex-col gap-3">
          <h2 className="font-semibold text-[#FFF5F8]">Join Sesi Partner</h2>
          <input
            value={joinCodeInput}
            onChange={e => setJoinCodeInput(e.target.value.toUpperCase())}
            placeholder="Kode sesi (12 huruf)"
            maxLength={12}
            className="rounded-xl border border-white/10 bg-[#18181C] px-4 py-3 text-sm text-[#FFF5F8] tracking-widest outline-none focus:border-[#FF3D7F]/60 transition"
          />
          <button
            onClick={handleJoin}
            disabled={loadingJoin || !joinCodeInput.trim()}
            className="w-full py-3 rounded-xl bg-white/10 hover:bg-white/15 text-[#FFF5F8] font-semibold transition disabled:opacity-50"
          >
            {loadingJoin ? "Bergabung..." : "Join"}
          </button>
        </div>
      </div>
    );
  ...
  if (phase === "setup") {
    return (
      <div className="mx-auto w-full max-w-md px-4 py-10 flex flex-col gap-6">
        <div>
          <button onClick={() => setPhase("idle")} className="text-xs text-[#5C5470] hover:text-[#9B93B0] transition">
            ← Kembali
          </button>
          <h1 className="mt-3 text-2xl font-bold text-[#FFF5F8]">Setup Game</h1>
        </div>
        ...
        {/* Total rounds */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5 flex flex-col gap-3">
          <label className="text-sm font-medium text-[#FFF5F8]">Jumlah Ronde</label>
            ...
            {([5, 7, 10] as const).map(n => (
              <button
                ...
                className={`py-3 rounded-xl border font-bold text-sm transition ${
                  totalRounds === n
                    ? "border-[#FF3D7F] bg-[#FF3D7F]/20 text-[#FF6B9D]"
                    : "border-white/10 bg-white/5 text-[#9B93B0] hover:border-white/20"
                }`}
              >
                {n} Ronde<br />
                <span className="text-xs font-normal opacity-70">{COIN_COSTS[n]} coin</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Dare level */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5 flex flex-col gap-3">
          <label className="text-sm font-medium text-[#FFF5F8]">Level Dare</label>
          <div className="flex flex-col gap-2">
            {(["sweet_only", "mixed", "full_chaos"] as const).map(level => (
              <button
                key={level}
                onClick={() => setDareLevel(level)}
                className={`py-3 px-4 rounded-xl border text-left text-sm transition ${
                  dareLevel === level
                    ? "border-[#FF3D7F] bg-[#FF3D7F]/10 text-[#FFF5F8]"
                    : "border-white/10 bg-white/5 text-[#9B93B0] hover:border-white/20"
                }`}
              >
        ...
        <button
          ...
          className="w-full py-4 rounded-2xl bg-[#FF3D7F] hover:bg-[#FF6B9D] text-white font-bold text-base transition disabled:opacity-50"
        >
