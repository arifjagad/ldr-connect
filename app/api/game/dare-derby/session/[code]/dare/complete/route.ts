 ...
 * Loser menandai dare selesai dilakukan → delegasi ke RPC complete_dare_derby_dare
 */
export async function POST(
  ...
  const { data: newState, error: rpcError } = await serviceClient.rpc("complete_dare_derby_dare", {
  ...
  if (rpcError) {
    ...
    if (msg.includes("NOT_THE_LOSER"))
      return NextResponse.json({ success: false, message: "Hanya yang kalah yang bisa menyelesaikan dare", data: null }, { status: 403 });
    if (msg.includes("DARE_NOT_PENDING"))
      return NextResponse.json({ success: false, message: "Dare bukan dalam status pending", data: null }, { status: 400 });
  ...
  return NextResponse.json({ success: true, message: "Dare ditandai selesai!", data: { game_state: newState as DareDerbyGameState } });
}
